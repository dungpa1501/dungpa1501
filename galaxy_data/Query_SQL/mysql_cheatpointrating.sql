-- insert cheat point vào table rating_detal

SET @movie = '16fbcb14-9755-4c85-bb47-a691b2c1e3e2';

INSERT INTO rating_detail 
SELECT UUID(), @movie, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36', 10, NOW(), NOW(), null, null, 'movie', null, null, 0
FROM (
...
-- update vao table rating

update rating a 
join movie b on a.entityId = b.id
join (
	select b.id,count(a.entityId) munb , sum(point) po
from rating_detail a 
join movie b on a.entityId = b.id
where b. startdate > CURRENT_DATE()
group by b.id
) c on c.id = b.id 
set a.totalVotes = c.munb, a.point = (c.po / c.munb)
where b. startdate > CURRENT_DATE()